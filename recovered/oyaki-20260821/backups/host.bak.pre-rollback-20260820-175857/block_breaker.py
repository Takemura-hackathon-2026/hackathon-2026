...
class PositionCalibration:
    """校正時のLEFT/CENTER/RIGHTをゲームの0/0.5/1へ対応付ける。"""
    ...
    def __post_init__(self) -> None:
        values = (self.left_x, self.center_x, self.right_x)
        if not all(math.isfinite(value) for value in values):
            raise ValueError("左右位置の校正値は有限値である必要がある")
        if not self.left_x < self.center_x < self.right_x:
            raise ValueError("左右位置の校正値は left < center < right である必要がある")

    def map(self, x: float, mirror: bool = False) -> float:
        """非対称な実測幅を保ち、基準姿勢を必ずゲーム中央へ置く。"""
        if x <= self.center_x:
            position = 0.5 * (x - self.left_x) / (self.center_x - self.left_x)
        else:
            position = 0.5 + 0.5 * (x - self.center_x) / (self.right_x - self.center_x)
        position = min(1.0, max(0.0, position))
        return 1.0 - position if mirror else position
...
class CameraController:
    ...
    def __init__(
        ...
        if self.play_range is not None and self.position_calibration is not None:
            raise ValueError("--play-range と校正済み位置範囲は同時に使用できない")
    ...
    def read(self, now: float) -> InputState:
        ...
        if not background_phase:
            ...
            if contours:
                # distance_probe.py の Detector.measure() と同じく、最大輪郭1つを測る。
        ...
        self.people_count = 1 if body is not None else 0
        if background_phase:
            return InputState()
        state = self.classifier.update(body, now)
        if body is not None:
            if self.position_calibration is not None:
                position = self.position_calibration.map(body.x, self.classifier.mirror)
            else:
                # 校正位置が無い古い起動方法だけ、生の位置と手動範囲を使用する。
            ...
            # ゲーム開始は校正済みのInputClassifierによる単一のジャンプ判定に限定する。
            state = InputState(state.lateral, state.jump, True, True,
                               position=min(1.0, max(0.0, position)))
...
class Calibration:
    ...
    def __init__(self, data: dict, path: Path, zone: str | None = None) -> None:
        ...
        if all(key in base for key in ("x", "y", "bottom", "area")):
            ...
            left_x = thresholds["left"].get("x_max")
            right_x = thresholds["right"].get("x_min")
            if left_x is not None and right_x is not None:
                self.position_calibration = PositionCalibration(
                    float(left_x), self.baseline.x, float(right_x)
                )
    ...
    def summary(self) -> str:
        return (
            ...
            + ("" if self.position_calibration is None else
               f"\n  position left={self.position_calibration.left_x:.4f} "
               f"center={self.position_calibration.center_x:.4f} "
               f"right={self.position_calibration.right_x:.4f}")
        )
...
def parser() -> argparse.ArgumentParser:
    ...
    result.add_argument("--camera-background-seconds", type=float, default=3.0)
    ...
    result.add_argument(
        ...
        default=None,
        metavar="LOW,HIGH",
        help="校正ファイルを使わない場合の手動横位置範囲。指定時は校正済みLEFT/CENTER/RIGHTより優先",
    )
    result.add_argument(
        "--position-gain",
        type=float,
        ...
        if not keyboard_mode:
            camera = CameraController(
                ...
                position_calibration=(
                    None
                    if calibration is None or args.play_range is not None
                    else calibration.position_calibration
                ),
            )
