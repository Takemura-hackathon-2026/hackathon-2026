...
def main() -> int:
    ...
    airborne = BodyMeasurement(.5, .455, .75, .2)
    gate.update(stance, .0554, .2445, 0.0)
    if gate.update(airborne, .0554, .2445, .2):
        errors.append("構える前の通過人物をジャンプとして受理する")
    gate.update(None, .0554, .2445, .3)
    for step in range(16):
        gate.update(stance, .0554, .2445, 1.0 + step / 30)
    if not gate.update(airborne, .0554, .2445, 1.55):
