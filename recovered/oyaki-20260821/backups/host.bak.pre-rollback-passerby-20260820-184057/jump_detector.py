...
from block_breaker import CameraController  # noqa: E402
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--preview", action="store_true", help="カメラ画像とマスクを表示")
...
def main(argv: list[str] | None = None) -> int:
    ...
    if (
        ...
        print("error: カメラ・閾値・フレーム指定が不正", file=sys.stderr)
        return 2
    ...
    try:
        ...
        print(
            "jump detector: "
            f"camera={args.camera} threshold_y={args.jump_rise_y_min:g} "
        ...
        while running and (args.frames <= 0 or frame_count < args.frames):
            ...
            if args.preview:
                if camera.debug is not None:
                    debug = camera.debug.copy()
                    ...
                    cv2.imshow("jump detector camera", debug)
            ...
            if now >= next_report:
                ...
                print(f"status stage={camera.stage} frames={frame_count} body={body_count} jumps={jump_count} fps={frame_count / elapsed:.1f}")
                next_report = now + 1.0
