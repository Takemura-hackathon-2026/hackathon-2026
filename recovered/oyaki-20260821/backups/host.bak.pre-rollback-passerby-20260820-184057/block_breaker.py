...
class JumpStartGate:
    """一時的に横切る人物を無視し、構えてからのジャンプだけを通す。"""

    STANCE_SECONDS = .45
    STABLE_Y_RANGE = .025
    STABLE_BOTTOM_RANGE = .060

    def __init__(self) -> None:
        self.samples: list[tuple[float, float, float]] = []
        self.rest_y: float | None = None
        self.rest_bottom: float | None = None
        self.armed = False

    def reset(self) -> None:
        self.samples.clear()
        self.rest_y = None
        self.rest_bottom = None
        self.armed = False
    ...
    def update(
        ...
        if body is None:
            ...
            return False
        if not self.armed:
            self.samples.append((now, body.y, body.bottom))
            cutoff = now - self.STANCE_SECONDS
            self.samples = [row for row in self.samples if row[0] >= cutoff]
            if self.samples and self.samples[0][0] <= cutoff + .04:
                ys = [row[1] for row in self.samples]
                bottoms = [row[2] for row in self.samples]
                if max(ys) - min(ys) <= self.STABLE_Y_RANGE and max(bottoms) - min(bottoms) <= self.STABLE_BOTTOM_RANGE:
                    self.rest_y = float(np.median(ys))
                    self.rest_bottom = float(np.median(bottoms))
                    self.armed = True
                else:
                    # 動き続ける通行人は窓を満たしても基準姿勢を作れない。
                    self.samples = self.samples[-1:]
            return False
        assert self.rest_y is not None and self.rest_bottom is not None
        # 実測した軽いジャンプを通しつつ、単なる立ち揺れより十分大きくする。
        rise_y_min = max(.030, jump_rise_y_min * .70)
        rise_bottom_min = max(.100, jump_rise_bottom_min * .60)
        jump = self.rest_y - body.y >= rise_y_min and self.rest_bottom - body.bottom >= rise_bottom_min
        if jump:
            self.reset()
            return True
        return False
...
class CameraController:
    ...
    def __init__(
        ...
        self.jump_start_gate = JumpStartGate()
    ...
    def read(self, now: float) -> InputState:
        ...
        jump = self.jump_start_gate.update(
            body,
            self.classifier.jump_rise_y_min,
            self.classifier.jump_rise_bottom_min,
            now,
        )
        if body is not None:
            ...
            state = InputState(state.lateral, jump, True, True,
    ...
    def _select(self, bodies: list[BodyMeasurement], now: float) -> BodyMeasurement | None:
        ...
        現在の操作者を維持し、居なくなったときだけ最大面積の人へ引き継ぐ。
        通行人の輪郭変化をジャンプと誤認して操作対象を奪わない。
        """
        people = self.tracker.update(bodies, now)
        if not people:
            self.active_uid = None
            self.jump_start_gate.reset()
            return None
        active = next((p for p in people if p.uid == self.active_uid), None)
        if active is None:
            active = max(people, key=lambda p: p.area)
            if self.active_uid is not None and self.active_uid != active.uid:
                self.jump_start_gate.reset()
