...
class JumpStartGate:
    ...
    def __init__(self) -> None:
        self.stance_since: float | None = None
        self.armed = False
    ...
    def update(
        ...
        if body is None or baseline is None:
            ...
            return False
        stance = (
            abs(body.y - baseline.y) < jump_rise_y_min * .45
            and abs(body.bottom - baseline.bottom) < jump_rise_bottom_min * .45
        )
        if stance:
            if self.stance_since is None:
                self.stance_since = now
            elif now - self.stance_since >= self.STANCE_SECONDS:
                self.armed = True
        else:
            self.stance_since = None
        if jump and self.armed:
            self.reset()
            return True
        return False
