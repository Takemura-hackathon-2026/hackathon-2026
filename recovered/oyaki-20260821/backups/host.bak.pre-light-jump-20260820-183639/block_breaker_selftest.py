...
def main() -> int:
    ...
    gate = JumpStartGate()
    baseline = BodyMeasurement(.5, .5, .9, .2)
    stance = BodyMeasurement(.5, .5, .9, .2)
    airborne = BodyMeasurement(.5, .42, .62, .2)
    gate.update(stance, baseline, .05, .24, False, 0.0)
    if gate.update(airborne, baseline, .05, .24, True, .2):
        errors.append("構える前の通過人物をジャンプとして受理する")
    gate.update(None, baseline, .05, .24, False, .3)
    gate.update(stance, baseline, .05, .24, False, 1.0)
    gate.update(stance, baseline, .05, .24, False, 1.5)
    if not gate.update(airborne, baseline, .05, .24, True, 1.55):
        errors.append("通常姿勢で構えた後のジャンプを受理しない")
    ...
    for step in range(50):
        # クリア待機中からジャンプ信号が残っている状況を再現する。
        game.step(.04, GameInput(launch=True), 1.5 + step * .04)
    ...
    game.step(.04, GameInput(launch=True), 3.6)
    if not game.serving:
        errors.append("リセット前から残るジャンプ信号で勝手に開始する")
    game.step(.04, GameInput(), 3.64)
    game.step(.04, GameInput(launch=True), 3.68)
    if game.serving:
        errors.append("リセット後の新しいジャンプで開始しない")
