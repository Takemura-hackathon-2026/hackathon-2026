# RGB LEDマトリックス縦型インタラクティブゲーム開発計画書
...
本計画では、3台のRaspberry Pi間の表示ずれを最重要リスクとして扱う。センサー処理、入力判定、ゲームロジック、描画、フレーム確定はUbuntu主機へ集約する。Pi側は完成済みパレットインデックス配列の受信、裏バッファ保持、外部同期、HUB75出力だけを担当する。
...
| Pi構成 | Pi 1台あたり6列×4段＝24枚、192×128ピクセル。3台を縦積み |
...
> **重要:** 3台のPiで同じゲーム状態を個別描画する方式は採用しない。主機が完成させたピクセル配列を送り、同一フレーム番号を物理同期信号で切り替える。
...
| 機器 | 担当処理 | 禁止・抑制する処理 |
|---|---|---|
| Ubuntu主機 | カメラ取得、前景抽出、左右・ジャンプ判定、ゲーム更新、192×384描画、パレット量子化、3分割、UDP送信、READY管理、M5制御 | Piごとに異なるゲーム状態を持たせない |
| Raspberry Pi 4 ×3 | UDP受信、チャンク再構成、CRC確認、裏バッファ保持、同期待機、HUB75出力 | ゲームロジック、画像生成、圧縮・解凍、動画プロセス起動 |
| M5StickC Plus | ハードウェアタイマーによる`FRAME_SYNC`、起動時`GLOBAL_START` | 画像処理、ゲーム判断、フレーム配信 |
| USBカメラ | 固定画角でプレイヤーを取得 | 人物映像のLED転送 |
| 1GbEスイッチ | 主機と3台のPiを接続する専用有線LAN | Wi-Fi経由のフレーム配信 |
...
主機は192×384の単一フレームバッファへ、背景、ブロック、パドル、ボール、スコア、演出をすべて描画する。描画終了後に指定パレットへ量子化し、縦方向へ128ピクセルずつ3分割する。
...
ゲーム本体とカメラ入力の実装前に、表示経路・パレット・3台境界・同期を検証する専用テストモードを設ける。主機のローカルファイル`test.wbmp`をType-0 WBMPとして読み込み、192×384の論理画面内をDVDプレイヤーのロゴのように反射移動させる。
...
| 出力 | 192×384の1byte/pixelインデックス画面を128行ずつ3分割 |
...
開発用の`test_mode/udp_preview.py`は、同一PC上で3領域を再結合し、3台分が同じ`frame_id`になった場合だけ表示する。これは本番Piクライアントの代替ではなく、主機の分割・ヘッダー・CRC・フレーム整合を先に確認するためのシミュレータとする。
...
| Pi 1台分 | 192×128×1 | 24,576 byte |
| 全体60fps | 73,728×60 | 約4.42 MB/s、約35.4 Mbps |
| Pi 1台60fps | 24,576×60 | 約1.47 MB/s |
...
4. 主機は3台すべてのREADYを確認する。
5. 主機がM5StickC Plusへ`ARM N`を送る。
6. M5が次の基準周期で`FRAME_SYNC`を出力する。
7. 3台が同じフレーム`N`を採用し、走査周期を再同期する。

3台のうち1台でもREADYでなければSYNCを出さず、全台が直前フレームを維持する。
...
├── frame_transport/      # 3分割、UDP、CRC、READY
...
| 3台のIP管理とUDP送信 | 流用・拡張 | トリガー文字列から固定バイナリフレームへ変更 |
| Pi側`chain=8`、`parallel=3` | 流用 | 現在の6×4パネル構成と一致 |
...
| 第2週 | 1byte/pixel、UDPチャンク、CRC、`frame_id`、3分割 | 3台で静止画とスクロールバーを表示し、混在なし |
