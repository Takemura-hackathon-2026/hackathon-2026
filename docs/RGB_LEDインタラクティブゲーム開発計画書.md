...
32×32 RGB LEDパネル72枚を6列×12段に配置し、論理解像度192×384ピクセルの縦型ディスプレーを構成する。参加者の左右移動およびジャンプを非接触で検出し、DOTキャラクターが縦スクロール型コースを登るインタラクティブゲームを実現する。
...
- プレイヤーがカメラ前を左右へ移動すると、DOTキャラクターが同方向へ移動する。
- プレイヤーのジャンプを検出すると、キャラクターが足場へ跳び移る。
- コースは縦方向へスクロールする。
- 人物映像や人影はLEDへ描画しない。
...
主機は192×384の単一フレームバッファへ、背景、足場、キャラクター、スコア、演出をすべて描画する。描画終了後に指定パレットへ量子化し、縦方向へ96ピクセルずつ4分割する。
